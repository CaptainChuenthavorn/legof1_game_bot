...
def ai_drive_loop(window_name: str, duration=180):
    monitor = get_client_area(window_name)
    with mss.mss() as sct:
        start_time = time.time()
        print("[*] AI driving...")
        ...
        while time.time() - start_time < duration:
            ...
            # ======== SIMPLE RULE: Follow the white line ========= #
            h, w = frame.shape[:2]
            center_x = w // 2
            sample_y = h // 2 + 50

            left_pixel = frame[sample_y, center_x - 30]
            right_pixel = frame[sample_y, center_x + 30]

            is_left_white = np.all(left_pixel > 200)
            is_right_white = np.all(right_pixel > 200)

            if is_left_white and not is_right_white:
                keyboard.press('left')
                keyboard.release('right')
            elif is_right_white and not is_left_white:
            ...
            # Debug view
            cv2.circle(frame, (center_x - 30, sample_y), 5, (255, 0, 0), -1)
            cv2.circle(frame, (center_x + 30, sample_y), 5, (0, 0, 255), -1)
