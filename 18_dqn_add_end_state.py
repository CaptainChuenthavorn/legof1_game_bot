...
class LEGOF1Env(gym.Env):
    ...
    def step(self, action):
        # Check if race has finished (score page appeared)
        try:
            if self.page:
                result_element = self.page.query_selector("#application > div.page.score > div > div.score__result > div.score__result__box > div.score__result__box__time")
                if result_element:
                    result_text = result_element.inner_text()
                    print(f"[✓] Finish state detected with time: {result_text}")
                    return self._get_frame(), 10, True, False, {"reason": "finished", "time": result_text}
                print("[✓] Finish state detected: score page visible")
                return self._get_frame(), 10, True, False, {"reason": "finished"}  # bonus reward on finish
        except Exception:
            pass
        if action == 0:
            keyboard.press('left')
...
async def setup_browser(env):
    ...
    env.attach_browser(pw, browser, page)
...
if __name__ == "__main__":
    ...
    obs, _ = env.reset()
    for _ in range(300):  # run for ~15 seconds
        action, _ = model.predict(obs, deterministic=True)
        obs, reward, terminated, truncated, info = env.step(action)
        env.render()
        if terminated or truncated:
            # Log result time if available
            if "time" in info:
